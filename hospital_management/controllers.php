...
function doctorCtrl($conn) {
    ...
    if ($action === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($title === '' || $author === '' || $quantity === '' || $price === '') {
            ...
            $error = 'Ward No must be a non-negative whole number.';
        } elseif (!is_numeric($price) || floatval($price) < 0) {
            $error = 'Fee must be a non-negative number.';
        } else {
            $docId = $_SESSION['user']['id'];
            if (addPatient($conn, $title, $author, intval($quantity), floatval($price), $docId)) {
    ...
    if ($action === 'update' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($title === '' || $author === '' || $quantity === '' || $price === '') {
            ...
            if (updatePatient($conn, $id, $title, $author, intval($quantity), floatval($price))) {
