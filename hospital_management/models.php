...
function getPatients($conn) {
    $r = mysqli_query($conn, "SELECT id, title, author, quantity, price FROM patients ORDER BY id DESC");
    return mysqli_fetch_all($r, MYSQLI_ASSOC);
}

function getPatient($conn, $id) {
    $stmt = mysqli_prepare($conn, "SELECT id, title, author, quantity, price FROM patients WHERE id = ?");
    mysqli_stmt_bind_param($stmt, 'i', $id);
    mysqli_stmt_execute($stmt);
    $row = mysqli_fetch_assoc(mysqli_stmt_get_result($stmt));
    mysqli_stmt_close($stmt);
...
function addPatient($conn, $title, $author, $quantity, $price, $doctorId) {
    $stmt = mysqli_prepare($conn,
        "INSERT INTO patients (title, author, quantity, price, doctor_id) VALUES (?, ?, ?, ?, ?)");
    mysqli_stmt_bind_param($stmt, 'ssidi', $title, $author, $quantity, $price, $doctorId);
    $ok = mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    return $ok;
}

function updatePatient($conn, $id, $title, $author, $quantity, $price) {
    $stmt = mysqli_prepare($conn,
        "UPDATE patients SET title = ?, author = ?, quantity = ?, price = ? WHERE id = ?");
...
function searchPatients($conn, $term) {
    $like = '%' . $term . '%';
    $stmt = mysqli_prepare($conn,
        "SELECT id, title, author, quantity, price FROM patients
